...
기능명 | 상세
-------|-----
카테고리 목록 | 게시판에 들어가기 전에 관심있는 카테고리를 검색과 태그로 검색 가능하며, 조회와 선택을 할 수 있다.
게시판 목록 | 비회원의 경우 전체 리스트 조회만 가능하며 로그인 시 상세정보 조회가 가능하다. 카테고리별로 선택할 수 있으며, 검색과 내가 쓴 글 검색도 가능하다. 게시글에 대한 댓글 수나 신청인원을 확인할 수 있다.
게시판 등록 | 소모임 모집을 선택하면 추가 정보를 기입할 수 있다.
게시판 상세 | 글을 읽고 댓글을 달 수 있다. 작성자는 수정 삭제가 가능하다. 소모임 모집 글의 경우 신청인원을 받을 수 있으며, 정해진 인원이 차면 소모임 게시글이 자동 생성된다. 자동 생성된 후 소모임 게시글은 관리자만 수정과 삭제가 가능하다. 소모임 게시글의 경우 참여한 인원이 아니면 조회가 불가능하다.
