...
기능명 | 상세
-------|-----
안전 상품 등록 | 관리자는 안전 상품을 등록, 수정, 삭제 할 수 있다.
안전 상품 리스트 | 회원은 안전 상품 리스트를 별점으로 볼 수 있고, 안전 상품을 전체,제목,내용으로 검색 가능하다. 그리고 최근 본 상품이 배열로 저장되어(최대 3개) 뷰단 왼 쪽에 표출된다.
안전 상품 상세 | 회원은 안전 상품에 대하여 리뷰를 쓸 수 있고, 별점을 줄 수 있다. 자신이 쓴 리뷰는 삭제,수정 할 수 있으며, 타인이 쓴 리뷰는 신고,추천 할 수 있다.
안전 지도| 회원은 안전 지도에서 마커를 누르고, 인포를 통해 관련 사건 정보와 링크를 타고 관련 뉴스로 갈 수 있으며, 댓글을 작성,조회(내용,작성자),삭제,수정할 수 있으며, 타인이 쓴 댓글을 추천할 수 있다.

### 취미 게시판
기능명 | 상세
-------|-----
카테고리 목록 | 게시판에 들어가기 전에 관심있는 카테고리를 검색과 태그로 검색 가능하며, 조회와 선택을 할 수 있다. 관리자는 등록, 수정, 삭제를 할 수 있다.
게시판 목록 | 비회원의 경우 전체 리스트 조회만 가능하며 로그인 시 상세정보 조회가 가능하다. 목록별로 확인할 수 있으며, 검색과 내가 쓴 글 검색도 가능하다. 게시글에 대한 댓글 수나 신청인원을 확인할 수 있다.
게시판 등록 | 파일을 등록할 수 있다. 소모임 모집을 선택하면 추가 정보를 기입할 수 있다.
게시판 상세 | 글을 읽고 댓글을 달 수 있다. 작성자는 수정과 삭제가 가능하다. 소모임 모집 글의 경우 신청인원을 받을 수 있으며, 정해진 인원이 차면 소모임 게시글이 자동 생성된다. 자동 생성된 후 소모임 게시글은 관리자만 수정과 삭제가 가능하다. 소모임 게시글의 경우 참여한 인원이 아니면 조회가 불가능하다.


### 관리자페이지/Notice게시판
기능명 | 상세
-------|-----
관리자페이지 | 카테고리별 db데이터를 가져와 수정 및 삭제를 쉽게 나타낸다.
뉴스룸 | 파싱을 통해 최신뉴스를 업데이트해서 가져온다.
